...
public sealed partial class SimulationWorld
{
    ...
    private const int ApeLumberCampWoodCapacity = 30;
    ...
    public const int ApeCombatFeedingCooldownTicks = 10 * TicksPerSecond;
    ...
    // Barbarian camps reuse the Village structure and placement rules, but not
    // the ordinary village lifecycle.
    private readonly HashSet<int> _barbarianVillageTiles = [];
    ...
    private readonly Dictionary<int, long> _apeFeedingBlockedUntilTicks = [];
    ...
    {
        var tileIndex = GetIndex(position);
        return GetApeVillageResidentCountByTile(tileIndex);
    }
    ...
    public int GetApeVillageCivilianCount(GridPosition position)
    {
        ...
        return _apeVillageHomes.Count(pair =>
            pair.Value == villageTile &&
            _critterIndicesById.TryGetValue(pair.Key, out var residentIndex) &&
            (_species[residentIndex] is CritterSpecies.Ape or CritterSpecies.ApeFarmer or
                CritterSpecies.ApeScholar or CritterSpecies.ApeLumberjack ||
                IsLivingApe(_species[residentIndex]) && _plagues.ContainsKey(pair.Key)));
    }
    ...
    public bool IsBarbarianVillage(GridPosition position) =>
        _barbarianVillageTiles.Contains(GetIndex(position));

    public int[] GetBarbarianPopulationBySpecies()
    {
    ...
    public bool IsBarbarianApe(CritterId critterId) =>
    ...
    public int GetApeVillageWoodCapacity(GridPosition position) =>
        _apeStructures.TryGetValue(GetIndex(position), out var structure) &&
            structure is ApeStructureKind.Village
                ? GetApeVillageWoodCapacityByTile(GetIndex(position))
                : 0;
    ...
    internal bool TryAssignApeToVillage(CritterId apeId, GridPosition villagePosition)
    {
        ...
        if (!_critterIndicesById.TryGetValue(apeId.Value, out var apeIndex) ||
            _species[apeIndex] is not (CritterSpecies.Ape or CritterSpecies.ApeFarmer or CritterSpecies.ApeLumberjack or CritterSpecies.ApeSailor or CritterSpecies.ApeWarrior or CritterSpecies.ApeScholar or CritterSpecies.ApeChieftain) ||
    ...
    public bool RemoveApeStructureAt(GridPosition position)
    {
        ...
        RemoveApeFarmStaff(tileIndex);
        RemoveApeLumberStaff(tileIndex);
        ...
        if (structure is not ApeStructureKind.Village)
        {
            ...
            if (hasVillage)
            {
                ...
                if (_apeVillageWood.TryGetValue(villageTile, out var wood))
                {
                    _apeVillageWood[villageTile] = Math.Min(wood, GetApeVillageWoodCapacityByTile(villageTile));
                }
            }
            return true;
        }
        ...
        _villageFeasts.Remove(tileIndex);
        _apeFoodExpansionTicks.Remove(tileIndex);
            ...
            .ToArray())
        {
            ...
            RemoveApeFarmStaff(auxiliaryTile);
            RemoveApeLumberStaff(auxiliaryTile);
    ...
    public CritterSnapshot GetCritter(int index)
    {
        ...
        return new CritterSnapshot(
            ...
            !isColonist && CanSpeciesReproduce(_species[index]) &&
                (_species[index] is not CritterSpecies.Vampire || HasVampireBirthSlot(index)) &&
        ...
        for (var index = 0; index < _count; index++)
        {
                ...
                : _species[index] switch
            {
                ...
                CritterSpecies.ApeChieftain =>
                    TryMoveHunter(
                        index,
                        ApeChieftainDefenderPerceptionRadius,
                        reservedPrey,
                        huntWhenFull: true),
                CritterSpecies.UndeadApe or CritterSpecies.Vampire => TryMoveHunter(index, ApePerceptionRadius, reservedPrey),
    ...
    private void AdvanceCritterLifecycles()
    {
        ...
        for (var index = 0; index < _count; index++)
        {
            ...
            if (!isStranded && (metabolized || Tick % TicksPerSecond == 0) &&
                species is (CritterSpecies.Ape or CritterSpecies.ApeFarmer or CritterSpecies.ApeLumberjack or CritterSpecies.ApeSailor or
                    CritterSpecies.ApeWarrior or CritterSpecies.ApeScholar or CritterSpecies.ApeChieftain or CritterSpecies.Dog))
            {
            ...
            if (!CanSpeciesReproduce(species) ||
                _energy[index] < nutrition.ReproductionThreshold)
            {
                _apeReproductionStalls.Remove(_critterIds[index].Value);
                continue;
            }

            if (species is CritterSpecies.Vampire)
            {
                if (!HasVampireBirthSlot(index))
                    continue;
                reservedBirthTiles ??= [];
                var child = FindBirthPosition(index, species, reservedBirthTiles);
                if (child is null)
                    continue;
                var lair = PrepareVampireLair(index);
                if (lair < 0 || GetVampireLairPopulation(GetPosition(lair)) +
                    (births?.Count(birth => birth.Species is CritterSpecies.Vampire && birth.ApeVillage == lair) ?? 0)
                    >= VampireLairCapacity)
                    continue;
                reservedBirthTiles.Add(child.Value);
                (births ??= []).Add((species, child.Value, species, lair));
                _energy[index] -= nutrition.ReproductionCost;
                continue;
            }

            if (species is CritterSpecies.Dog)
            {
                if (!_apeVillageHomes.TryGetValue(_critterIds[index].Value, out var dogVillage) ||
                    GetVillageDogCount(dogVillage) + (births?.Count(birth =>
                        birth.Species is CritterSpecies.Dog && birth.ApeVillage == dogVillage) ?? 0) >= GetVillageDogLimit(dogVillage))
                    continue;
                reservedBirthTiles ??= [];
                var puppyPosition = FindBirthPosition(index, CritterSpecies.Dog, reservedBirthTiles);
                if (puppyPosition is { } puppy)
                {
                    reservedBirthTiles.Add(puppy);
                    (births ??= []).Add((CritterSpecies.Dog, puppy, CritterSpecies.Dog, dogVillage));
                    _energy[index] -= nutrition.ReproductionCost;
                }
                continue;
            }
        ...
        if (births is not null)
        {
            foreach (var birth in births)
            {
                ...
                if (added && birth.Species is (CritterSpecies.Ape or CritterSpecies.Dog) && birth.ApeVillage >= 0)
                {
                    var offspringIndex = _occupants[GetIndex(birth.Position)];
                    if (offspringIndex >= 0 && _species[offspringIndex] is CritterSpecies.Ape or CritterSpecies.Dog)
                    {
                        _apeVillageHomes[_critterIds[offspringIndex].Value] = birth.ApeVillage;
                    }
                }
                if (added && birth.Species is CritterSpecies.Vampire)
                    _vampireHomes[_critterIds[_occupants[GetIndex(birth.Position)]].Value] = birth.ApeVillage;
            }
        }
    }
    ...
    private bool TryPrepareApeReproduction(int apeIndex, CritterNutrition nutrition)
    {
        ...
        if (hasVillage)
        {
            return CanApeReproduceAtCurrentPosition(_critterIds[apeIndex], villageTile);
        }
    ...
    internal bool CanApeReproduceAtCurrentPosition(CritterId apeId, int villageTile)
    {
        if (!_critterIndicesById.TryGetValue(apeId.Value, out var index) ||
            !IsLivingApe(_species[index]) ||
            !_apeVillageHomes.TryGetValue(apeId.Value, out var home) || home != villageTile ||
            _apeSettlerTargets.ContainsKey(apeId.Value) ||
            _energy[index] < CritterNutritions.Get(_species[index]).ReproductionThreshold ||
            GetApeVillageResidentCountByTile(villageTile) >=
                GetApeVillagePopulationCapacityByTile(villageTile))
        {
            _apeReproductionStalls.Remove(apeId.Value);
            return false;
        }
        if (IsAtOrAdjacentToConnectedApeStructure(index, villageTile))
        {
            _apeReproductionStalls.Remove(apeId.Value);
            return true;
        }
        var position = _positions[index];
        if (!_apeReproductionStalls.TryGetValue(apeId.Value, out var stall) ||
            stall.VillageTile != villageTile || stall.Position != position)
        {
            _apeReproductionStalls[apeId.Value] = (villageTile, position, Tick);
            return false;
        }
        return Tick - stall.SinceTick >= ApeReproductionStallTicks;
    }

    private bool TryFeedApeFromVillage(int apeIndex, CritterNutrition nutrition)
    {
        // Stored food must not replenish energy drained by an active plague.
        if (IsApeFeedingBlocked(apeIndex) || _plagues.ContainsKey(_critterIds[apeIndex].Value) ||
            _energy[apeIndex] > nutrition.HungryThreshold ||
    ...
    internal void FeedApeFromVillageSurplus(int villageTile)
    {
        ...
        if (!_apeVillageFood.TryGetValue(villageTile, out var food) || food <= 0 ||
            food * 10 < GetApeVillageFoodCapacityByTile(villageTile) * 9)
        {
            return;
        }

        if (!_apeVillageGrowthFeedTargets.TryGetValue(villageTile, out target))
        {
            var candidates = _apeVillageHomes.Keys.Where(IsEligible).Order().ToArray();
            if (candidates.Length == 0)
            {
                return;
            }
            target = candidates[NextInt(candidates.Length)];
            _apeVillageGrowthFeedTargets[villageTile] = target;
        }
    ...
    private bool CanBuildApeStructureOn(int tileIndex, ApeStructureKind kind) =>
        !_vampireLairs.Contains(tileIndex) &&
    ...
    private int GetApeVillageFoodCapacityByTile(int villageTile) =>
        _barbarianVillageTiles.Contains(villageTile)
            ? BarbarianVillageFoodCapacity
            : ApeVillageBaseFoodCapacity +
                ApeFoodDistrictStorageCapacity * GetApeFoodDistrictCount(villageTile);

    private int GetApeVillageWoodCapacityByTile(int villageTile) =>
        _barbarianVillageTiles.Contains(villageTile) ? 0 : ApeMaximumWood + ApeLumberCampWoodCapacity *
            GetApeStructureCount(villageTile, ApeStructureKind.LumberCamp);
    ...
    private void AdvanceApeVillages()
    {
            ...
            .ToArray())
        {
            if (_barbarianVillageTiles.Contains(villageTile))
            {
                ...
                {
                    // Camps use the ordinary one-chief rule, but none of the
                    // civilian infrastructure or recruitment progression.
                    TryRecruitApeChieftain(villageTile);
                    TryRecruitBarbarianPirate(villageTile);
                }
                continue;
            }
            var population = GetApeVillageResidentCountByTile(villageTile);
            ...
            CheckVillageRoadPopulation(villageTile, population);
            TryBuildEmergencyApeFarm(villageTile);

            if (population >= ApeLibraryPopulationThreshold &&
                !HasApeStructure(villageTile, ApeStructureKind.Library))
            {
                TryPurchaseApeStructure(villageTile, ApeStructureKind.Library);
            }

            TrySendApeSettler(villageTile, population);
            var infrastructureLimit = GetApeInfrastructureLimitForPopulation(population);

            // Colonists need food production before growing beyond two residents.
            if (population >= 2 &&
            ...
            if (population >= ApeFoodDistrictPopulationThreshold &&
                (population < GetApeVillagePopulationCapacityByTile(villageTile) ||
                    !_apeVillageWood.TryGetValue(villageTile, out var availableWood) ||
                    availableWood < ApeResidentialWoodCost) &&
                GetApeStructureCount(villageTile, ApeStructureKind.LumberCamp) <
                    GetApeLumberCampLimitForPopulation(population))
            {
                ...
                .ToArray())
            {
                ...
                else if (structure is ApeStructureKind.Library)
                {
                    AdvanceApeLibrary(villageTile, structureTile);
                }
            }
            ...
            {
                FeedApeFromVillageSurplus(villageTile);
            }
    ...
    /// <summary>Seeds a normal two-ape village for manual testing.</summary>
    public bool TrySpawnTestApeVillage(GridPosition position)
        => TrySpawnApeVillage(position, isBarbarian: false);

    /// <summary>Seeds a barbarian two-ape village for manual testing.</summary>
    public bool TrySpawnBarbarianApeVillage(GridPosition position)
        => TrySpawnApeVillage(position, isBarbarian: true);

    private bool TrySpawnApeVillage(GridPosition position, bool isBarbarian)
    {
        var villageTile = GetIndex(position);
        if ((isBarbarian && _terrain[villageTile] is Terrain.Shallows) ||
            !TryGetApeVillageAuxiliarySite(-1, villageTile, out _, out _))
        {
            return false;
        }
        ...
        if (isBarbarian)
        {
            _barbarianVillageTiles.Add(villageTile);
            _apeVillageWood.Remove(villageTile);
        }

        for (var index = 0; index < residents.Length; index++)
        {
            var residentId = AddCritter(residents[index], GetPosition(spawnTiles[index]));
            _apeVillageHomes[residentId.Value] = villageTile;
        }
        return true;
    }

    private bool TryFoundBarbarianCamp(int villageTile)
    {
        if (_terrain[villageTile] is Terrain.Shallows || !ShouldFoundBarbarianCamp(villageTile))
        {
            return false;
        }
    ...
    private bool ShouldFoundBarbarianCamp(int villageTile) =>
        (Seed ^ ((ulong)(uint)villageTile * 2_654_435_761UL)) % 100 <
        NaturalBarbarianVillageChancePercent;

    private void TryRecruitBarbarianPirate(int villageTile)
    {
        if (!IsBarbarianCampCoastal(villageTile) || _apeVillageHomes.Any(pair =>
            pair.Value == villageTile && _critterIndicesById.TryGetValue(pair.Key, out var residentIndex) &&
            _species[residentIndex] is CritterSpecies.ApeSailor))
        {
            return;
        }
    ...
    private bool IsBarbarianCampCoastal(int villageTile)
    {
        if (_terrain[villageTile] is Terrain.Beach)
        {
            return true;
        }
        var village = GetPosition(villageTile);
        return MovementDirections.Any(direction =>
        {
            var y = village.Y + direction.Y;
            return y >= 0 && y < Height &&
                _terrain[GetIndex(new GridPosition(Mod(village.X + direction.X, Width), y))] is Terrain.Shallows;
        });
    }
        ...
        int requestedTargetVillageTile = -1)
    {
        ...
        RememberApeColonistTechnologies(settler.Value, villageTile);
    ...
    internal static int GetApeLumberCampLimitForPopulation(int population) =>
        1 + Math.Max(0, population - 1) / ApeLumberCampPopulationInterval;
    ...
    private bool TryBuildApeFoodDistrict(int villageTile) =>
        TryPurchaseApeStructure(villageTile, ApeStructureKind.Farm) ||
        TryPurchaseApeStructure(villageTile, ApeStructureKind.RicePaddy) ||
        TryPurchaseApeStructure(villageTile, ApeStructureKind.Orchard) ||
        TryPurchaseApeStructure(villageTile, ApeStructureKind.Aquaculture);
    ...
    internal bool TryPurchaseApeStructure(int villageTile, ApeStructureKind kind)
    {
        _apeVillageFood.TryGetValue(villageTile, out var food);
    ...
    private void BuildApeStructureAt(int villageTile, int constructionTile, ApeStructureKind kind)
    {
        ...
        {
            // Let recruitment and the first harvest finish before assessing another shortage.
            _apeFoodExpansionTicks[villageTile] = Tick + 60 * TicksPerSecond;
    ...
    private bool TryLaunchLoneApeSailorColonist(int villageTile, int population)
    {
        ...
        RememberApeColonistTechnologies(sailorId, villageTile);
    ...
    {
        AdvanceApeFarmerRecruitment(villageTile, farmTile, structure);
        if (!HasApeFarmer(farmTile))
        {
            _apeStructureNextActionTicks[farmTile] = Tick + GetApeFoodProductionIntervalTicks(farmTile, structure);
            return;
        }
    ...
    private void AdvanceApeLumberCamp(int villageTile, int lumberCampTile)
    {
        ...
        AdvanceApeLumberjackRecruitment(villageTile, lumberCampTile);
        if (!HasApeLumberjack(lumberCampTile))
        {
            _apeStructureNextActionTicks[lumberCampTile] = Tick + interval;
            return;
        }
    ...
    private bool TryRecruitApeChieftain(int villageTile)
    {
        ...
        if (hasChieftain)
        {
            // A subsequent vacancy begins a fresh recruitment cooldown.
            _apeStructureNextActionTicks.Remove(villageTile);
            return false;
        }

        if (!_apeStructureNextActionTicks.TryGetValue(villageTile, out var nextTick))
        {
            _apeStructureNextActionTicks[villageTile] = Tick + ApeChieftainRecruitmentIntervalTicks;
            return false;
        }
        if (Tick < nextTick)
        {
    ...
    internal CritterSpecies ChooseOffspringSpecies(CritterSpecies parentSpecies)
    {
        ...
        if (parentSpecies is CritterSpecies.ApeSailor or CritterSpecies.ApeWarrior or
            CritterSpecies.ApeChieftain or CritterSpecies.ApeFarmer or CritterSpecies.ApeLumberjack)
        {
        ...
        bool preserveApeVillage = false)
    {
        ...
        _chieftainAttackTargets.Remove(critterId);
        DetachVampireFromLair(critterId);
        ...
        GridPosition? preferredTarget = null)
    {
        ...
        var target = preferredTarget ?? FindHunterPrey(
            critterIndex,
            predatorSpecies,
            perceptionRadius,
            reservedPrey);
        ...
        RememberChieftainTarget(critterIndex, _occupants[GetIndex(target.Value)]);
        var current = _positions[critterIndex];
    ...
    private GridPosition? TryMoveApe(
        ...
        IReadOnlySet<GridPosition>? reservedPrey)
    {
        var workerId = _critterIds[critterIndex].Value;
        var workplace = _species[critterIndex] switch
        {
            CritterSpecies.ApeFarmer => _apeFarmers.FirstOrDefault(
                pair => pair.Value == workerId && HasApeFarmer(pair.Key), new(-1, -1)).Key,
            CritterSpecies.ApeLumberjack => _apeLumberjacks.FirstOrDefault(
    ...
    private void MoveApeSettlerToward(int apeIndex, int targetVillageTile)
    {
        ...
        if (!_apeSettlerPaths.TryGetValue(apeId, out var route) ||
            ...
            !IsApeSettlerTransitTile(apeIndex, route.Tiles.Peek()))
        {
        ...
        CritterSpecies? pathSpecies = null)
    {
        ...
        var order = 0;
        frontier.Enqueue((startTile, 0), (!sailorRoute ? GetApeSettlerPathEstimate(startTile, targetTile) : 0, order++));
        var reachedTile = -1;
            ...
            foreach (var direction in MovementDirections)
            {
                ...
                if (!(pathSpecies is { } species ? CanLiveOn(species, neighborTile) :
                    ...
                    (_occupants[neighborTile] >= 0 && _occupants[neighborTile] != apeIndex &&
                        !CanShoveMovementBlocker(apeIndex, neighborTile, reservedPrey)) ||
                ...
                frontier.Enqueue(
                    ...
                    (nextSteps + (!sailorRoute ? GetApeSettlerPathEstimate(neighborTile, targetTile) : 0), order++));
            }
        }
        ...
        for (var tile = reachedTile; tile != startTile; tile = cameFrom[tile])
        {
        ...
        IReadOnlySet<GridPosition>? reservedPrey)
    {
        ...
        {
            if (_species[critterIndex] is CritterSpecies.ApeSailor)
            {
                MoveApeSailorTowardVillage(_critterIds[critterIndex], villageTile, reservedPrey);
                return true;
            }
            var target = FindNearestConnectedApeStructure(critterIndex, villageTile);
            if (target >= 0)
            {
                TryMoveTowardApeStructure(critterIndex, target, reservedPrey);
            }
        }
        // Stay near the village so the lifecycle pass can create the offspring.
        return true;
    }

    internal void MoveApeSailorTowardVillage(CritterId sailorId, int villageTile,
        IReadOnlySet<GridPosition>? reservedPrey = null)
    {
        if (!_critterIndicesById.TryGetValue(sailorId.Value, out var index) ||
        ...
        foreach (var structureTile in EnumerateConnectedApeStructureTiles(villageTile))
        {
            var position = GetPosition(structureTile);
            foreach (var direction in CardinalDirections.Append(new GridPosition(0, 0)))
            {
                var y = position.Y + direction.Y;
                if (y < 0 || y >= Height)
                {
                ...
                var tile = GetIndex(new GridPosition(Mod(position.X + direction.X, Width), y));
                if (CanLiveOn(CritterSpecies.ApeSailor, tile))
                {
                    goals.Add(tile);
                }
            }
        ...
        if (goals.Count == 0 || goals.Contains(current))
        {
            _apeSailorReturnPaths.Remove(sailorId.Value);
            return;
        }
        if (!_apeSailorReturnPaths.TryGetValue(sailorId.Value, out var route) ||
            route.VillageTile != villageTile || route.Tiles.Count == 0 ||
            !goals.Contains(route.Tiles.Last()) ||
            GetApeSettlerPathEstimate(current, route.Tiles.Peek()) != 1 ||
            !CanLiveOn(CritterSpecies.ApeSailor, route.Tiles.Peek()) ||
            reservedPrey?.Contains(GetPosition(route.Tiles.Peek())) is true ||
            (_occupants[route.Tiles.Peek()] >= 0 &&
                !CanShoveMovementBlocker(index, route.Tiles.Peek(), reservedPrey)))
        {
            var path = FindApeSettlerPath(index, current, villageTile, goals, reservedPrey);
            if (path is null || path.Count == 0)
            {
                _apeSailorReturnPaths.Remove(sailorId.Value);
                return;
            }
            route = (villageTile, path);
            _apeSailorReturnPaths[sailorId.Value] = route;
        }
        var next = route.Tiles.Peek();
    ...
    internal void CommitEncounter(GridPosition predatorPosition, GridPosition preyPosition)
    {
        ...
        RememberChieftainTarget(predatorIndex, preyIndex);
        ...
        if (predatorSpecies != preySpecies &&
            ...
            (CanEat(preySpecies, predatorSpecies) ||
                ...
                predatorSpecies is (CritterSpecies.MegaSpider or CritterSpecies.ToothedWhale) &&
                    IsPredator(preySpecies)))
        {
            CommitMutualPredatorCombat(predatorPosition, preyPosition);
            return;
        }
        ...
        GridPosition defenderPosition)
    {
        ...
        if (defenderWins)
        {
            ApplyCombatDamage(defenderIndex, attackerIndex);
            _damageFlashUntilTicks[attackerIndex] = Tick + CombatDamageFlashTicks;
            BlockApeFeedingAfterDamage(attackerIndex);
        ...
        ApplyCombatDamage(attackerIndex, defenderIndex);
    ...
    private void BlockApeFeedingAfterDamage(int index)
    {
        if (IsLivingApe(_species[index]) || _species[index] is CritterSpecies.Dog)
            _apeFeedingBlockedUntilTicks[_critterIds[index].Value] = Tick + ApeCombatFeedingCooldownTicks;
    }

    private bool IsApeFeedingBlocked(int index) => (IsLivingApe(_species[index]) || _species[index] is CritterSpecies.Dog) &&
        _apeFeedingBlockedUntilTicks.TryGetValue(_critterIds[index].Value, out var until) && Tick < until;
    ...
    private void FeedPredatorAt(GridPosition predatorPosition, CritterSpecies preySpecies)
    {
        ...
        if (predatorSpecies is CritterSpecies.Vampire)
            return;
        if (preySpecies is CritterSpecies.Wolf)
            TryRecruitDogFromWolfKill(predatorIndex);
        ...
        if (IsBarbarianApe(predatorIndex))
        {
        ...
        {
            if (IsBarbarianApe(predatorIndex) &&
                _apeVillageHomes.TryGetValue(apeId, out var barbarianVillageTile))
            {
                AddApeVillageFood(barbarianVillageTile, settlementFood);
            }
            else
            {
    ...
    internal static bool CanEat(CritterSpecies predator, CritterSpecies prey) =>
        CanEatSpecies(predator is CritterSpecies.Dog ? CritterSpecies.ApeWarrior : predator,
            prey is CritterSpecies.Dog ? CritterSpecies.ApeWarrior : prey);

    private static bool CanEatSpecies(CritterSpecies predator, CritterSpecies prey) =>
        ...
        predator switch
    {
        ...
        CritterSpecies.SeaScorpion =>
            prey is CritterSpecies.Fish or CritterSpecies.Worm or CritterSpecies.Trilobite or CritterSpecies.Newt or
                ...
                CritterSpecies.Therapsid or CritterSpecies.Monkey or CritterSpecies.Ape or CritterSpecies.ApeFarmer or CritterSpecies.ApeLumberjack or
                CritterSpecies.Wolf or CritterSpecies.ApeWarrior or CritterSpecies.ApeScholar or CritterSpecies.ApeChieftain or
                CritterSpecies.ApeSailor or CritterSpecies.Deer or CritterSpecies.Elk or
                CritterSpecies.Gazelle,
        CritterSpecies.MegaSpider => true,
        ...
        CritterSpecies.ApeChieftain => prey is not CritterSpecies.Vampire && CanEatSpecies(CritterSpecies.Ape, prey),
        CritterSpecies.Wolf =>
    ...
    private bool CanEatInCurrentContext(int predatorIndex, int preyIndex)
    {
        ...
        if (_species[preyIndex] is CritterSpecies.Vampire &&
            _species[predatorIndex] is (CritterSpecies.ApeWarrior or CritterSpecies.ApeChieftain or CritterSpecies.Dog))
            return false;
        if (IsChieftainEscortTarget(predatorIndex, preyIndex))
            return true;
        ...
        if (IsBarbarianApe(predatorIndex))
        {
            return !_apeVillageHomes.TryGetValue(_critterIds[preyIndex].Value, out var preyVillage) ||
                !_apeVillageHomes.TryGetValue(_critterIds[predatorIndex].Value, out var predatorVillage) ||
                preyVillage != predatorVillage;
        }
        ...
        {
            return CanMegaSpiderCannibalize(predatorIndex);
        }
        ...
        if (_species[predatorIndex] is not (CritterSpecies.Ape or CritterSpecies.ApeFarmer or CritterSpecies.ApeLumberjack) ||
    ...
    private bool IsBarbarianApe(int critterIndex) =>
        _apeVillageHomes.TryGetValue(_critterIds[critterIndex].Value, out var villageTile) &&
        _barbarianVillageTiles.Contains(villageTile);

    private bool AreRivalApeFactions(int firstIndex, int secondIndex) =>
        IsLivingApe(_species[firstIndex]) && IsLivingApe(_species[secondIndex]) &&
        IsBarbarianApe(firstIndex) != IsBarbarianApe(secondIndex);
    ...
    internal static bool CanFightBackAgainst(CritterSpecies defender, CritterSpecies attacker) =>
        defender switch
        {
            CritterSpecies.BaleenWhale => attacker is CritterSpecies.ApeSailor,
    ...
    private int GetCombatDamage(int critterIndex) =>
        (IsBarbarianApe(critterIndex) ? GetCombatDamage(CritterSpecies.ApeWarrior) - 1 :
    ...
    internal static bool IsPredator(CritterSpecies species) => species is
        ...
        CritterSpecies.Therapsid or CritterSpecies.Ape or CritterSpecies.ApeFarmer or CritterSpecies.ApeLumberjack or CritterSpecies.ApeSailor or CritterSpecies.ApeWarrior or CritterSpecies.ApeChieftain or
        CritterSpecies.UndeadApe or CritterSpecies.Vampire;
    ...
    private static bool IsApePredator(CritterSpecies species) => species is
        ...
        CritterSpecies.Wolf or CritterSpecies.UndeadApe;
    ...
    private static bool IsLargeLandPredatorPrey(CritterSpecies prey) =>
        prey is CritterSpecies.Trilobite or CritterSpecies.Nautilus or
            ...
            CritterSpecies.Wolf or CritterSpecies.Ape or CritterSpecies.ApeFarmer or CritterSpecies.ApeLumberjack or CritterSpecies.ApeSailor or
    ...
    private static bool IsToothedWhaleShallowsPrey(CritterSpecies prey) => prey is
        CritterSpecies.Dog or
        CritterSpecies.MegaToad or CritterSpecies.MegaSpider or CritterSpecies.Therapsid or CritterSpecies.Ape or CritterSpecies.ApeFarmer or CritterSpecies.ApeLumberjack or
        CritterSpecies.ApeWarrior or CritterSpecies.ApeScholar or CritterSpecies.ApeChieftain or
        CritterSpecies.Deer or CritterSpecies.Elk or CritterSpecies.Gazelle or
        CritterSpecies.Wolf;
    ...
    {
        if (species is (CritterSpecies.Nautilus or CritterSpecies.Trilobite) &&
            IsIceOverDeepOcean(tileIndex))
        {
            return -1;
        }
    ...
    private bool IsTrilobiteFeedingTile(int tileIndex) =>
        IsTrilobiteFeedingTerrain(_terrain[tileIndex]);

    private bool IsWormFeedingTile(int tileIndex) =>
        IsTrilobiteFeedingTile(tileIndex) || IsIceOverDeepOcean(tileIndex);
    ...
    private bool IsNautilusFeedingTile(int tileIndex) =>
        IsNautilusFeedingTerrain(_terrain[tileIndex]);
    ...
    private bool RemoveCritterAtIndex(int critterIndex)
    {
        ...
        _apeFeedingBlockedUntilTicks.Remove(removedId.Value);
    ...
    internal static int GetMovementIntervalTicks(CritterSpecies species) => species switch
    {
        ...
        CritterSpecies.Worm => 7 * TicksPerSecond,
        ...
        CritterSpecies.ApeScholar => 3 * TicksPerSecond,
        CritterSpecies.UndeadApe or CritterSpecies.Vampire => 3 * TicksPerSecond,
        CritterSpecies.Deer => 3 * TicksPerSecond,
        CritterSpecies.Elk => 4 * TicksPerSecond,
        CritterSpecies.Gazelle => 3 * TicksPerSecond,
        CritterSpecies.Wolf or CritterSpecies.Dog => 5 * TicksPerSecond / 2,
    ...
    private long GetFirstMovementTick(CritterSpecies species, CritterId id)
    {
        ...
        return species is CritterSpecies.Fish or CritterSpecies.Nautilus or
            ...
            CritterSpecies.Therapsid or CritterSpecies.Monkey or CritterSpecies.Ape or CritterSpecies.ApeFarmer or CritterSpecies.ApeLumberjack or
            CritterSpecies.ApeSailor or CritterSpecies.ApeWarrior or CritterSpecies.ApeScholar or CritterSpecies.ApeChieftain or CritterSpecies.UndeadApe or CritterSpecies.Wolf or CritterSpecies.ToothedWhale or
            CritterSpecies.BaleenWhale or CritterSpecies.Dog
